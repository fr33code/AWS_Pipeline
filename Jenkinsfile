        ...
        stage('Build') {
            steps {
                ...
                withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'aws-key', usernameVariable: 'AWS_ACCESS_KEY_ID', passwordVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                        ...
                        sh """\
                    aws s3 cp /var/jenkins_home/workspace/aws_pipeline/build/libs/java-sample-app-v5.jar s3://sonuajayin/apps/java-sample-app-v5.jar
                    aws elasticbeanstalk create-application-version \
                        --application-name "datasearch_demo" \
                        --version-label "v5" \
                        --source-bundle "S3Bucket=sonuajayin,S3Key=apps/java-sample-app-v5.jar" \
                        --process \
                        --auto-create-application \
                        --query 'ApplicationVersion.VersionLabel' \
                        --output text
                    aws elasticbeanstalk update-environment --environment-name DatasearchDemo-env --application-name datasearch_demo --version-label v5
                    """
                    // sh "
